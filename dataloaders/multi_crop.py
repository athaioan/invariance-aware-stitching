...
class MultiCropAugmentation(object):
    def __init__(self, global_number, global_scale, local_number, local_scale, make_gray=False):
        ...
        normalize_list = [transforms.ToTensor(),
                          transforms.Normalize((0.485, 0.456, 0.406), (0.229, 0.224, 0.225)),]
        
        if make_gray:
            normalize_list.append(transforms.Grayscale(num_output_channels=3))
...
class MultiCropWrapper(nn.Module):
    ...
    def forward(self, x, return_feat=None):
        # convert to list
        ...
        start_idx, output, feature = 0, torch.empty(0).to(x[0].device), torch.empty(0).to(x[0].device)  

        if return_feat:

            for end_idx in idx_crops:
                _out, _feat = self.model(torch.cat(x[start_idx:end_idx]), return_feat=return_feat)
                # accumulate outputs
                output = torch.cat((output, _out))
                feature = torch.cat((feature, _feat))
                start_idx = end_idx
