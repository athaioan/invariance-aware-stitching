...
def train_one_epoch(model, clf_loss, data_loader, optimizer, epoch, attack, args):
    ...
    ## wrap optimizer 
    if args.use_pcgrad:
        optimizer = PCGrad(optimizer, reduction='sum') ## reduction='sum' to match regular loss aggregation
    ...
    for it, (image, label)  in enumerate(metric_logger.log_every(data_loader, 10, header)):
        ...
        param_groups = optimizer._optim.param_groups if args.use_pcgrad else optimizer.param_groups
        ...
        if args.direct_matching:
            ...
            if args.use_pcgrad:
                ...
                losses = [args.beta_hint * hint_loss,   
                        args.beta_functional * functional_loss, 
                        args.beta_soft * soft_targets_loss, 
                        args.beta_cls * classification_loss]
        ...
        with torch.no_grad():
            ...
            metric_logger.update(lr=param_groups[0]["lr"])
            metric_logger.update(wd=param_groups[0]["weight_decay"])
