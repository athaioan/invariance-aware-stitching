...
def get_metric_from_folder(paths, ds, metric):
    ...
    for yaml_path in yaml_paths:
        ...
        if 'vit_base_patch16_224_vit_base_patch16_224' in yaml_path and 'adversarial' in yaml_path:
            layer_keys = [0, 2, 4, 6, 8]
        else:
            layer_keys = list(range(num_layers))
        ...
        for i in layer_keys:
            try:
                if is_cka:
                    ...
                    current_metric.append((metrics[i][ds][metric] - 1/NUM_CLASSES*100)/(100 - 1/NUM_CLASSES*100)*100)  # normalize accuracy
            except:
...
def plot_results(result_dict, datasets_to_plot, metrics_to_plot, front_model_id=None,end_model_id=None, stitch_title=None, figure_name=None, plot_y=False):
    ...
    for metric_index, (data, metric) in enumerate(zip(datasets_to_plot, metrics_to_plot)):
        ...
        for mode_index, mode in enumerate(result_dict.keys()):
            ...
            plt.xticks(range(len(metric_mean)), x_labels, size=18)
    ...
    y_label = 'Accuracy (%)' if metric == 'top1' else 'Similarity (%)' if metric == 'agreement_prob' else None
...
if __name__ == '__main__':
    ...
    METRIC_TO_PLOT = ['agreement_prob'] 
    
    DATA_MODES = ['Real', 'IRIs']

    
    STITCHING_MODES = {'Real': ['CKA', 'FuLA'],
                       'IRIs': ['CKA', 'FuLA'],}
    ...
    for metric in METRIC_TO_PLOT:
        ...
        for exp_id, (stitch_title, front_model_id, end_model_id, exp_path_list) in enumerate(EXP_TO_RUN[DATASET]):
            ...
            for exp_path, seed in exp_path_list:
                ...
                for data_mode in DATA_MODES:
                    ...
                    for stitching_mode_ in stitching_modes:
                        ...
                        elif stitching_mode_ == 'CKA':
                            RESULT_DICT_OUTER.setdefault(data_mode, {}).setdefault('CKA', []).append(f'{os.path.dirname(EXP_DATA_PATHS[data_mode])}/{CKA_path}/{seed}')
            ...
            first_legend_args, second_legend_args = plot_results(result_dict=RESULT_DICT_OUTER,
                                                                ...
                                                                figure_name=f'{metric}_cka.png',
                                                                plot_y = (exp_id==0) )
            # except:
            #     print(f"Could not plot {stitch_title} for {metric}")
            

        # Collect all saved figure paths
        image_paths = [os.path.join(path, f'{metric}_cka.png') for path in list_of_paths if os.path.exists(os.path.join(path, f'{metric}_cka.png'))]
        ...
        if image_paths:
            ...
            spacing_x = 50  # horizontal space in pixels
            ...
            first_legend = ax.legend(**first_legend_args,
                                    ...
                                    markerscale=1.5 * 5/6,
                                    handlelength=2 * 5/6,
                                    handleheight=1.0 * 5/6,    
                                    labelspacing=0.5 * 5/6,    
                                    columnspacing=2.5 * 5/6,
                                    borderpad=0.5 * 5/6,      
                                    handletextpad=1.0 * 5/6,)
            ...
            os.makedirs(f'./construct_figures/figures/CKA/{ARCH}/{DATASET}', exist_ok=True)


            # Save with legends
            plt.savefig(f'./construct_figures/figures/CKA/{ARCH}/{DATASET}/{metric}_cka.png', 
                        bbox_inches='tight', 
                        dpi=100,
                        facecolor='white')
