...
from utils import get_args, init_distributed_mode, fix_random_seeds, print_program_info, write_metrics_to_yaml, get_wandb_log, unwrap, plot_data_samples
...
from training.stitching.trainer_pc import train_one_epoch
...
def run_cross_layer_stitching(args):
    ...
    def _setup_output_directory(args):
        ...
        base_path = [args.output_dir,
                     ...
                     f"{args.front_id}_{args.end_id}",
                     f"{args.init_transform}_K_init_{args.K_init}_DM_{args.direct_matching}_RS_{args.bn_running_stats}",
                     f"front_index_{args.front_layer_index}",
                     f"{args.seed}"]
       
        if args.iris:
            base_path[4] = base_path[4] + f"_iris_ratio_{args.iris_attack_ratio}_K_{args.K_iris}"

        if args.included_class_ratio:
            base_path[4] = base_path[4] + f"_included_class_ratio_{args.included_class_ratio}"
               
        if args.robust:
            base_path.insert(-3, f"atttack_{args.robust}_ratio_{args.attack_ratio}")

        if args.backdoor:
            base_path.insert(-3, f"backdoor_{args.backdoor}_type_{args.backdoor_type}_loc_{args.backdoor_loc}_noise_{args.backdoor_noise}_first_{args.backdoor_first}")
    ...
    for stitch_layer_index in stitch_indices: 
        ...
        args.stitch_layer_index = stitch_layer_index 

        args.iris_first_time = (args.stitch_layer_index == stitch_indices[1])
        args.skip_training = True if args.stitch_layer_index == stitch_indices[0] or args.stitch_layer_index == stitch_indices[-1] else False
...
def train_stitching(args):
    ...
    if args.skip_training:
        ...
        for key, loader, eval_func in [
                                       ('ind', args.val_loader, eval),                                       
        ...
        if args.iris:
            ...
            if args.experiment_mode == "cross_stitching" and not(args.iris_first_time):
                print("IRIS have been generated already.")
                pass
            else:
                setup_iris_loaders(args, model, iris_attack)
        ...
        for epoch in range(0, args.epochs):
            ...
            # ============ training one epoch ... ============
            train_stats['ind'] = train_one_epoch(model, criterion, args.train_loader,
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--config_file",
        default="./configs/config_stitch.yaml",
        type=str,
    ...
    elif args.experiment_mode == "cross_stitching":
        aggregate_metrics = run_cross_layer_stitching(args)
    else:
