...
print(f"Running SimCLR with backbone {BACKBONE}")
print(f"BATCH_SIZE = {BATCH_SIZE}, N_EPOCHS = {N_EPOCHS}")
print(f"BASE_LR = {BASE_LR}, WEIGHT_DECAY = {WEIGHT_DECAY}, GRAYSCALE_PROB = {GRAYSCALE_PROB}")
print(f"MOMENTUM = {MOMENTUM}")
print(f"PROJECTOR_HIDDEN_SIZE = {PROJECTOR_HIDDEN_SIZE}, PROJECTOR_OUTPUT_SIZE = {PROJECTOR_OUTPUT_SIZE}")
print(f"CROP_LOW_SCALE = {CROP_LOW_SCALE}")
...
# N_EPOCHS = 50
# N_CPU_WORKERS = 16
# BASE_LR = 0.1         # important
# WEIGHT_DECAY = 1e-5    # important
# MOMENTUM = 0.9
# PROJECTOR_HIDDEN_SIZE = 1024
# PROJECTOR_OUTPUT_SIZE = 128
# CROP_LOW_SCALE = 0.2
...
cifar10_train = CIFAR10(
    root="/mimer/NOBACKUP/groups/alvis_cvl/datasets", train=True, transform=transforms.ToTensor()
)

cifar10_test = CIFAR10(
    root="/mimer/NOBACKUP/groups/alvis_cvl/datasets", train=False, transform=transforms.ToTensor()
)

transforms_ssl = transforms.Compose(
    [
        transforms.RandomResizedCrop(size=32, scale=(CROP_LOW_SCALE, 1)),
        # transforms.RandomCrop(32, padding=4), ## TODO: check if this is close to 50%
...
cifar10_train_ssl = CIFAR10(root="/mimer/NOBACKUP/groups/alvis_cvl/datasets", train=True, transform=paired_ssl_transforms)
...
cifar10_train_classifier = CIFAR10(
    root="/mimer/NOBACKUP/groups/alvis_cvl/datasets", train=True, download=False, transform=transforms_classifier
)
